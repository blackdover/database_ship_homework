/*
 * =========================================
 * 港口集装箱运营系统 (TOS) 完整数据库初始化脚本
 * 包含：创建数据库、创建表、创建视图
 * MySQL 5.7+ / 8.0+
 * =========================================
 * 
 * ⚠️ 警告：此脚本会删除整个数据库！
 * 如果数据库已存在且包含 Django 系统表（如 auth_user, django_migrations 等），
 * 这些表将被删除。请确保：
 * 1. 这是全新安装，或者
 * 2. 你已经备份了所有数据
 * 
 * 如果只想创建业务表而不删除 Django 系统表，请使用 init_tables_only.sql
 */

-- =========================================
-- 第一步：创建数据库
-- ⚠️ 警告：这会删除整个数据库，包括 Django 系统表！
-- =========================================
...
-- 完成提示
-- =========================================
SELECT '数据库 box_management 创建成功！' AS 提示信息;
SELECT '所有表已创建完成！' AS 提示信息;
